import ast, os, copy
import sys
sys.path.append("/Users/zhangzejunzhangzejun/PycharmProjects/pythonProject/python-ast-explorer-master/extract_transform_complicate_code_new/")
...
def traverse_cur_layer(tree, code_list, ass_init_list):
    ...
    if hasattr(tree, "body"):
        for ind,child in enumerate(tree.body):
            ...
            traverse_cur_layer(child, code_list, ass_init_list)
def transform_for_else_code(content):
    code_pair_list = []
    # print(content)
    try:
        ...
        for tree, class_name in ana_py.func_def_list:
            ...
            for code in new_code_list:
                ...
                code_pair_list.append([ast.unparse(code[5])+"\n"+ast.unparse(code[3])+"\n"+ast.unparse(code[6]),  ast.unparse(code[4]) if code[7] else ast.unparse(code[5])+"\n"+ast.unparse(code[4]),line_list])
