...
def get_star(content):
    ...
    try:
        ...
        for tree, class_name in ana_py.func_def_list:
            ...
            for ind, (node, new_node_list, complete_code,*remain) in enumerate(code_list):
                line_list = []
                line_list.append([node.lineno, node.end_lineno])
                # complete_code="\n".join([new_node_list[1], new_node_list[0], new_node_list[2]])
                complete_code="\n".join([new_node_list[-1][-1],new_node_list[1], new_node_list[0], new_node_list[2]]) if len(new_node_list)>3 else "\n".join([new_node_list[1], new_node_list[0], new_node_list[2]])
                # line_list.append([node.lineno, node.end_lineno])
