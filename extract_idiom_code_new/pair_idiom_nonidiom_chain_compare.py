...
from extrac_idiom_chained_comparison import get_idiom_chained_comparison_improve
...
def save_to_csv(save_complicated_code_dir_pkl):
    ...
    util.save_pkl(util.data_root + "idiom_code_dir_star_1000_csv/", "chain_compare_idiom_code_shuffle",
                  result_compli_for_else_list)
    print("len of code: ", len(result_compli_for_else_list))
    util.save_csv(util.data_root + "idiom_code_dir_star_1000_csv/chain_compare_idiom_code.csv",
                  result_compli_for_else_list[:],
...
def parse_a_tree(tree):
    code_list = []
    old_code_list = get_idiom_chained_comparison_improve(tree)
    for code in old_code_list:
        code_list.append(transform_idiom_chain_compare(code))
    print(code_list)
    return code_list
    ...
    for file_info in dict_repo_file_python[repo_name]:
        ...
        try:
            ...
            for tree, class_name in ana_py.func_def_list:
                ...
                old_code_list = get_idiom_chained_comparison_improve(tree)
                for code in old_code_list:
                    each_code_pair = transform_idiom_chain_compare(code)
                    if each_code_pair:
...
def get_chain_compare(content):
    ...
    try:
        ...
        for tree, class_name in ana_py.func_def_list:
            ...
            for ind, (node, new_node) in enumerate(code_list):
                line_list = []
                line_list.append([node.lineno, node.end_lineno])
                complete_code="\n".join([new_node[1], new_node[0], new_node[2]])
                # line_list.append([node.lineno, node.end_lineno])
...
'''
    ...
    save_complicated_code_dir_pkl = util.data_root + "idiom_code_dir_pkl_new/chained_comparison_idiom_code/"
